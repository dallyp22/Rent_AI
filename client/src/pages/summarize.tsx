...
import { ArrowRight, ArrowLeft, Loader2, CheckCircle, XCircle, Download, TrendingUp, TrendingDown, AlertCircle, BarChart3, Building2, Home, Percent } from "lucide-react";
...
// Interface for the session summary endpoint response
interface SessionSummaryResponse {
  sessionId: string;
  sessionName: string;
  sessionDescription?: string;
  generatedAt: string;
  subjectProperties: PropertyWithVacancy[];
  competitorProperties: PropertyWithVacancy[];
  portfolioMetrics: {
    totalPortfolioUnits: number;
    avgPortfolioVacancy: number;
    avgCompetitorVacancy: number;
    portfolioVacancyByBedroom: {
      studio: number | null;
      '1BR': number | null;
      '2BR': number | null;
      '3BR': number | null;
    ...
    marketPosition: "Below Market" | "Above Market" | "At Market";
...
interface PropertyWithVacancy {
  ...
  scrapedUnitsCount: number;
  propertyUnitsCount: number;
  amenities?: string[];
  builtYear?: number;
  url?: string;
  distance?: number;
...
// Helper function for vacancy rate color coding
const getVacancyColor = (vacancyRate: number | null | undefined): { bg: string; text: string; border: string } => {
...
// Helper function for vacancy badge color
const getVacancyBadgeVariant = (vacancyRate: number | null | undefined): "default" | "secondary" | "outline" | "destructive" => {
  if (vacancyRate === null || vacancyRate === undefined) return "outline";
  if (vacancyRate <= 5) return "default";
  if (vacancyRate <= 15) return "secondary";
  return "destructive";
};
...
export default function Summarize({ params }: { params: { id?: string; sessionId?: string } }) {
  ...
  // NEW: Session summary query with vacancy calculations - POST endpoint
  const sessionSummaryQuery = useQuery<SessionSummaryResponse>({
    queryKey: ['/api/session/summary', params.sessionId],
    queryFn: async () => {
      const response = await apiRequest('POST', `/api/session/summary`, { sessionId: params.sessionId });
      if (!response.ok) {
        throw new Error('Failed to fetch session summary with vacancy data');
      }
      return await response.json();
    },
    enabled: isSessionMode && !!params.sessionId,
    ...
    <div className="space-y-6" data-testid="summarize-page">
      ...
      {isSessionMode ? (
        <div className="space-y-6">
          ...
          {/* NEW: Vacancy Metrics from Session Summary - Enhanced Display */}
          {sessionSummaryQuery.data && (
            <div className="space-y-6" data-testid="session-vacancy-metrics">
              {/* Portfolio Summary Section */}
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <BarChart3 className="h-5 w-5 text-primary" />
                    <h3 className="text-xl font-semibold">Portfolio Vacancy Metrics</h3>
                  </div>
                  <Badge variant="secondary" className="text-xs">
                    {sessionSummaryQuery.data.subjectProperties.length} Properties Analyzed
                  </Badge>
                </div>

                {/* Portfolio Overview Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {/* Average Portfolio Vacancy */}
                  <div className={`p-4 rounded-lg border ${getVacancyColor(sessionSummaryQuery.data.portfolioMetrics.avgPortfolioVacancy).bg} ${getVacancyColor(sessionSummaryQuery.data.portfolioMetrics.avgPortfolioVacancy).border}`}>
                    <div className="flex items-center space-x-2 mb-2">
                      <Building2 className="h-4 w-4 text-primary" />
                      <span className={`text-sm font-medium ${getVacancyColor(sessionSummaryQuery.data.portfolioMetrics.avgPortfolioVacancy).text}`}>
                        Avg Portfolio Vacancy
                      </span>
                    </div>
                    <div className="text-2xl font-bold">
                      {sessionSummaryQuery.data.portfolioMetrics.avgPortfolioVacancy.toFixed(1)}%
                    </div>
                    <div className="text-xs mt-1 opacity-80">
                      Across all subject properties
                    </div>
                  </div>

                  {/* Average Competitor Vacancy */}
                  <div className="p-4 bg-slate-50 dark:bg-slate-900/20 rounded-lg border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                      {sessionSummaryQuery.data.portfolioMetrics.avgCompetitorVacancy.toFixed(1)}%
                    </div>
                  ...
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                    <div className="flex items-center space-x-2 mb-2">
                      {sessionSummaryQuery.data.portfolioMetrics.marketPosition === "Below Market" ? (
                    ...
                    <div className="text-lg font-bold text-blue-900 dark:text-blue-100">
                      {sessionSummaryQuery.data.portfolioMetrics.marketPosition}
                    </div>
                    <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                      {Math.abs(sessionSummaryQuery.data.portfolioMetrics.performanceGap).toFixed(1)}% gap
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                    ...
                    <div className="text-2xl font-bold text-purple-900 dark:text-purple-100">
                      {sessionSummaryQuery.data.portfolioMetrics.totalPortfolioUnits}
                    </div>
              ...
              {/* Individual Property Vacancy Cards */}
              <div className="space-y-4">
                <h3 className="text-lg font-semibold">Property-Level Vacancy Analysis</h3>
                {sessionSummaryQuery.data.subjectProperties.map((property) => (
                  <Card key={property.id} data-testid={`property-vacancy-card-${property.id}`}>
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <CardDescription>
                            {property.address} • {property.totalUnits} Total Units
                          </CardDescription>
                        </div>
                        <Badge 
                          variant={getVacancyBadgeVariant(property.overallVacancy)}
                          className="text-sm"
                          data-testid={`vacancy-badge-${property.id}`}
                        >
                          <Percent className="h-3 w-3 mr-1" />
                          {property.overallVacancy !== null ? `${property.overallVacancy.toFixed(1)}%` : 'N/A'}
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {/* Overall Vacancy Display */}
                        <div className={`p-3 rounded-lg ${getVacancyColor(property.overallVacancy).bg} ${getVacancyColor(property.overallVacancy).border} border`}>
                          <div className="flex items-center justify-between">
                            <span className={`text-sm font-medium ${getVacancyColor(property.overallVacancy).text}`}>
                              Overall Vacancy Rate
                            </span>
                            <span className="text-lg font-bold">
                              {property.overallVacancy !== null ? `${property.overallVacancy.toFixed(1)}%` : 'No data available'}
                            </span>
                          </div>
                        </div>

                        {/* Vacancy by Bedroom Type */}
                        {property.vacancyByBedroom && (
                          <div>
                            <div className="text-sm font-medium mb-2 text-muted-foreground">
                              Vacancy by Bedroom Type
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                              {Object.entries(property.vacancyByBedroom).map(([bedroomType, vacancy]) => {
                                if (vacancy === null) return null;
                                const colors = getVacancyColor(vacancy);
                                return (
                                  <div 
                                    key={bedroomType}
                                    className={`p-2 rounded text-center ${colors.bg} ${colors.border} border`}
                                    data-testid={`bedroom-vacancy-${property.id}-${bedroomType}`}
                                  >
                                    <div className={`text-xs font-medium ${colors.text}`}>
                                      {bedroomType === 'studio' ? 'Studio' : bedroomType}
                                    </div>
                            ...
                            {Object.values(property.vacancyByBedroom).every(v => v === null) && (
                              <div className="text-sm text-muted-foreground text-center py-2">
                                No bedroom-level data available
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t">
                          <span>
                            {property.scrapedUnitsCount > 0 
                              ? `${property.scrapedUnitsCount} units scraped` 
                              : 'No scraped data'}
                          </span>
                          {property.builtYear && (
          ...
          {/* Loading state for session summary */}
          {sessionSummaryQuery.isLoading && (
            <Card data-testid="vacancy-metrics-loading">
              <CardHeader>
          ...
          {/* Error state for session summary */}
          {sessionSummaryQuery.isError && (
            <Card data-testid="vacancy-metrics-error">
              <CardHeader>
                <CardTitle className="text-red-600 flex items-center gap-2">
                  <XCircle className="h-5 w-5" />
                  Failed to Load Vacancy Metrics
                </CardTitle>
                <CardDescription>
                  Unable to retrieve vacancy data from the session summary
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                  {sessionSummaryQuery.error?.message || 'An error occurred while fetching vacancy metrics'}
                </p>
                <Button
                  onClick={() => sessionSummaryQuery.refetch()}
                  variant="outline"
                  size="sm"
                  disabled={sessionSummaryQuery.isFetching}
                >
                  {sessionSummaryQuery.isFetching ? (
                    <>
                      <Loader2 className="h-3 w-3 mr-2 animate-spin" />
                      Retrying...
                    </>
