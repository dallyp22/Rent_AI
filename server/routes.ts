...
Based on this data, provide exactly 3 specific, actionable insights that would help a property manager optimize their competitive position. Each insight should be concise (under 100 characters) and directly actionable. Format as a JSON array of strings.`;
  ...
  // POST endpoint for session summary with vacancy calculations
  app.post("/api/session/summary", isAuthenticatedAny, async (req: any, res) => {
    try {
      console.log('[SESSION_SUMMARY] ===========================================');
      console.log('[SESSION_SUMMARY] Starting session summary generation with vacancy calculations');
      ...
      console.log('[SESSION_SUMMARY] Session ID:', sessionId);
      ...
      console.log('[SESSION_SUMMARY] Subject properties:', subjectProfiles.length);
      console.log('[SESSION_SUMMARY] Competitor properties:', competitorProfiles.length);
      ...
      // Import vacancy calculation utilities
      const { calculateOverallVacancy, calculateVacancyByBedroom } = await import("@shared/vacancy-utils");
      
      // Process subject properties with enhanced vacancy calculations
      const subjectPropertiesWithVacancy = await Promise.all(
        subjectProfiles.map(async (profile) => {
          // Get scraping jobs for this profile
          const scrapingJobs = await storage.getScrapingJobsByProfile(profile.id);
          
          // Get scraped units for all scraped properties
          let allScrapedUnits: any[] = [];
          for (const job of scrapingJobs) {
            if (job.status === 'completed') {
              const scrapedProperties = await storage.getScrapedPropertiesByJob(job.id);
              for (const scrapedProp of scrapedProperties) {
                const units = await storage.getScrapedUnitsByProperty(scrapedProp.id);
                allScrapedUnits = allScrapedUnits.concat(units);
              }
            }
          }
          
          // Calculate overall vacancy
          const overallVacancy = calculateOverallVacancy(profile, allScrapedUnits);
          
          // Calculate vacancy by bedroom type
          const vacancyByBedroom = calculateVacancyByBedroom(profile, allScrapedUnits);
          ...
          return {
            ...
            unitMix: profile.unitMix || {},
            overallVacancy: overallVacancy !== null ? overallVacancy : 0,
            vacancyByBedroom: {
              studio: vacancyByBedroom.studio,
              '1BR': vacancyByBedroom.oneBedroom,
              '2BR': vacancyByBedroom.twoBedroom,
              '3BR': vacancyByBedroom.threeBedroom,
              '4BR+': vacancyByBedroom.fourPlusBedroom
            },
            scrapedUnitsCount: allScrapedUnits.length,
            propertyUnitsCount: propertyUnits.length,
            amenities: profile.amenities || [],
      ...
      // Process competitor properties with enhanced vacancy calculations
      const competitorPropertiesWithVacancy = await Promise.all(
      ...
      const calculatePortfolioMetrics = () => {
        const totalSubjectUnits = subjectPropertiesWithVacancy.reduce((sum, prop) => sum + (prop.totalUnits || 0), 0);
        const avgSubjectVacancy = subjectPropertiesWithVacancy.length > 0
          ? subjectPropertiesWithVacancy.reduce((sum, prop) => sum + prop.overallVacancy, 0) / subjectPropertiesWithVacancy.length
          : 0;
          
        const avgCompetitorVacancy = competitorPropertiesWithVacancy.length > 0
          ? competitorPropertiesWithVacancy.reduce((sum, prop) => sum + prop.overallVacancy, 0) / competitorPropertiesWithVacancy.length
          : 0;
          
        // Calculate average vacancy by bedroom type across portfolio
        const portfolioVacancyByBedroom = {
          studio: null as number | null,
          '1BR': null as number | null,
          '2BR': null as number | null,
          '3BR': null as number | null,
          '4BR+': null as number | null
        };
        
        const bedroomTypes = ['studio', '1BR', '2BR', '3BR', '4BR+'];
        for (const type of bedroomTypes) {
          const propertiesWithThisType = subjectPropertiesWithVacancy.filter(
            p => p.vacancyByBedroom[type as keyof typeof p.vacancyByBedroom] !== null
          );
          if (propertiesWithThisType.length > 0) {
            const sum = propertiesWithThisType.reduce(
              (acc, p) => acc + (p.vacancyByBedroom[type as keyof typeof p.vacancyByBedroom] || 0),
              0
            );
            portfolioVacancyByBedroom[type as keyof typeof portfolioVacancyByBedroom] = 
              Math.round((sum / propertiesWithThisType.length) * 100) / 100;
          }
        }
        
        return {
          totalPortfolioUnits: totalSubjectUnits,
          avgPortfolioVacancy: Math.round(avgSubjectVacancy * 100) / 100,
          avgCompetitorVacancy: Math.round(avgCompetitorVacancy * 100) / 100,
          portfolioVacancyByBedroom,
          marketPosition: avgSubjectVacancy < avgCompetitorVacancy ? "Below Market" :
                         avgSubjectVacancy > avgCompetitorVacancy ? "Above Market" : "At Market",
          performanceGap: Math.round((avgSubjectVacancy - avgCompetitorVacancy) * 100) / 100
        };
      };
      ...
      const response = {
        ...
        subjectProperties: subjectPropertiesWithVacancy,
        competitorProperties: competitorPropertiesWithVacancy,
        portfolioMetrics
      };
      
      console.log('[SESSION_SUMMARY] Summary generation completed successfully');
      console.log('[SESSION_SUMMARY] ===========================================');
      
      res.json(response);
      
    } catch (error) {
      console.error("[SESSION_SUMMARY] Error generating session summary:", error);
      res.status(500).json({ 
